...
from ..runner.events import ExecutionEvent, Initialized
from . import ServiceClient, events
from .constants import REPORT_VERSION, STOP_MARKER, WORKER_JOIN_TIMEOUT
...
def start(
    client: ServiceClient, api_name: Optional[str], location: str, base_url: str, in_queue: Queue, out_queue: Queue
) -> None:
    ...
    try:
        ...
        event = events.SuccessfulUpload(message=response.message, next_url=response.next_url)
        out_queue.put(event)
